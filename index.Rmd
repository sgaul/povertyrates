...
gh$city <- reorder(gh$city, -gh$poppovertyrate)
ggplot(data = gh, aes(x = period, y = poppovertyrate)) + 
  geom_line() +
  labs(y = NULL, x = NULL) + 
  scale_x_continuous(limits = c(1970,2010), breaks = c(1980,1990,2010)) +
  theme_minimal() +
  facet_wrap(~ city, ncol = 4) +
  ggtitle("Poverty rates since 1970 (Greater Hartford region)")
```
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 8}
povertycomp <- read.csv('povertycomp.csv')
ggplot(data = melt(povertycomp, measure.vars = c('poverty200','childpoverty'))) + 
  geom_point(aes(y = poverty100, x = value)) + 
  labs(x = NULL, y = '% under poverty line') + 
  scale_x_continuous(labels = percent) +
  scale_y_continuous(labels = percent) +
  theme_bw() +
  facet_grid(. ~ variable, labeller = label_value)
```

We can also convert poverty rates to look at the number of people living in poverty. 

```{r echo = FALSE, warning = FALSE, message = FALSE}
ggplot(data = ddply(subset(gh_sum,period >= 1980),.(period), summarise, Population.in.poverty = sum(sumpoor, na.rm = T)),
         aes(y = Population.in.poverty, x = period)) + 
  geom_line() + 
  ylim(0,80000) + 
  labs(x = "Year", y = 'Population', title = 'Total region population in poverty') +
  theme_minimal()
```
...
ggplot(aes(y = sumpoor, x = period), data = subset(gh_sum,period >= 1970)) + 
  ...
  labs(x = "Year", y = 'Population', title = 'Population in poverty by location') +
  theme_minimal()  
```
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.height = 8}
all <- read.csv("resurgentcitiesdata.csv")
pop1950 <- subset(all, period == 1950)
maxpop <- tapply(pop1950$population, pop1950$city, max, na.rm = TRUE)
all$index = all$population / maxpop[all$city]

qplot(data = all, x = period, y = poppovertyrate, geom = "line") + 
  labs(y = NULL, x = "Year") + 
  scale_x_continuous(limits = c(1970,2010)) +
  scale_y_continuous(limits = c(0,35)) +
  theme_minimal() +
  facet_wrap(~ city, ncol = 4) +
  ggtitle("Poverty trends since 1970 (older industrial cities)")
```
...
allnew <- subset(all, period == 2010)
ggplot(data = allnew, aes(y = index, x = poppovertyrate / 100)) +
  geom_text(aes(label = city, size = population / 45000), 
            position = position_jitter(width = .01, height = .02)) +
  stat_smooth(method = lm, se = FALSE) +
  scale_x_continuous(labels = percent) +
  scale_y_continuous(labels = percent) +
  guides(size = FALSE) + 
  theme_minimal() + 
  labs(y = "Population change 1950 - 2010", x = "Poverty rate (2010)", title = "Population growth (decline) vs. poverty rate, older industrial cities")
```
